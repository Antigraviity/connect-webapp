...
import { FiArrowLeft, FiUser, FiMail, FiPhone, FiMapPin, FiSave, FiCamera, FiCrop, FiX, FiTrash2, FiCalendar } from "react-icons/fi";
...
export default function EditServiceProfilePage() {
    ...
    // Image Upload State (Reuse same logic for consistency or simplify if needed)
    // For brevity, using simpler image logic here but ideally should match crop modal if desired. 
    // I'll reuse the crop modal logic to keep it premium.
    ...
    // ... Copying crop logic for consistency ... 
        ...
        const fetchProfile = async () => {
            try {
                ...
                if (prefData.success && prefData.preferences) {
                    ...
                    if (p.serviceAvailability) {
                        setProfile(prev => ({
                            ...prev,
                            weekdays: p.serviceAvailability.weekdays !== false,
                            weekends: p.serviceAvailability.weekends === true
                        }));
                    }
                }
            } catch (error) {
        ...
        <div className="p-6 max-w-4xl mx-auto">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex-1">
                    <h1 className="text-2xl font-bold text-gray-900">Edit Service Profile</h1>
                    <p className="text-gray-500 text-sm">Manage your contact info and service preferences</p>
                </div>
            </div>
            ...
            {/* Contact Details */}
            <div className="bg-white rounded-2xl border border-gray-100 p-8 mb-6">
                <h2 className="text-lg font-bold text-gray-900 mb-6 flex items-center gap-2"><FiUser className="w-5 h-5 text-blue-600" />Contact Details</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="md:col-span-2">
                        <label className="block text-sm font-medium text-gray-700 mb-2">Full Name</label>
                        <input type="text" value={profile.name} onChange={(e) => setProfile({ ...profile, name: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 transition-all outline-none" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Email Address</label>
                        <input type="email" value={profile.email} onChange={(e) => setProfile({ ...profile, email: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 transition-all outline-none" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Primary Phone</label>
                        <input type="tel" value={profile.phone} onChange={(e) => setProfile({ ...profile, phone: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 transition-all outline-none" />
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-8 mb-6">
                ...
                <div className="space-y-6">
                    <div>
                        ...
                        <input type="text" value={profile.street} onChange={(e) => setProfile({ ...profile, street: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 transition-all outline-none" />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">City</label>
                            <input type="text" value={profile.city} onChange={(e) => setProfile({ ...profile, city: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 transition-all outline-none" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">State</label>
                            <input type="text" value={profile.state} onChange={(e) => setProfile({ ...profile, state: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 transition-all outline-none" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-2">ZIP Code</label>
                            <input type="text" value={profile.zip} onChange={(e) => setProfile({ ...profile, zip: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-blue-500 transition-all outline-none" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-8 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <label className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all ${profile.weekdays ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}>
                        <span className={`font-semibold ${profile.weekdays ? 'text-green-800' : 'text-gray-700'}`}>Weekdays (9 AM - 6 PM)</span>
                        <input type="checkbox" checked={profile.weekdays} onChange={(e) => setProfile({ ...profile, weekdays: e.target.checked })} className="w-5 h-5 text-blue-600 rounded" />
                    </label>
                    <label className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all ${profile.weekends ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}>
                        <span className={`font-semibold ${profile.weekends ? 'text-green-800' : 'text-gray-700'}`}>Weekends (10 AM - 4 PM)</span>
                        <input type="checkbox" checked={profile.weekends} onChange={(e) => setProfile({ ...profile, weekends: e.target.checked })} className="w-5 h-5 text-blue-600 rounded" />
                    </label>
                </div>
            </div>
