...
export default function ServiceEarnings() {
  ...
  const fetchEarnings = async () => {
    try {
      ...
      const response = await fetch(`/api/vendor/earnings?sellerId=${user.id}&type=SERVICE`);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Service Bookings Revenue</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <p className="text-xs text-gray-500 mt-1">{earnings.totalOrders} bookings</p>
        </div>
        ...
        {chartData.length > 0 && chartData.some(d => d.revenue > 0) ? (
          ...
          <div className="flex items-center justify-center h-64 bg-gray-50 rounded-lg">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-400">Complete bookings to see your revenue trend</p>
            </div>
          </div>
        )}
      </div>
              ...
              <tbody>
                {recentOrders.map((order) => (
                  <tr key={order.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${order.status === "Delivered" || order.status === "Completed"
